...
define(['mozart'], function (mozart) {
	let instructionData = mozart(function(prototype, _, _protected, __, __private) {
		prototype.ru = [
...
В JavaScript правильный способ: <code>let <i>переменная</i> = <i>значение</i>;</code>. Ключевое здесь — слово <b>var</b>.<br>
<b>var</b> означает: «Создай мне переменную с именем: ». Таким образом, предыдущее выражение можно разделить на две части:<br>
	<code>let <i>переменная</i>;</code> и <code><i>переменная</i> = <i>значение</i>;</code>.<br><br>
...
<div class="code">let rollsOfToiletPaper = 5;
...
<div class="code">let milkExpired = false;
if (milkExpired) {
...
<div class="code">let greet = function(name) {
...
<div class="code">let fruits = ['яблоко', 'банан', 'груша'];</div>
...
<div class="code">for (let i = 0; i < fruits.length; i++) {
...
<div class="code">function loop(robot) {
	let currentValue = robot.info().someProperty;
...
4. В открывшемся файле напишите код модуля.
...
<div class="code">let queue = require('action-queue');
...
<div class="code">let closest = require('find').closestNext(x, y, items);</div>
...
<h3>Пример использования</h3>
<div class="code">let closestLift = require('find').closestNext(robot.info().x, robot.info().y, Game.lifts);</div>
