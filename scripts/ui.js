...
prevlevelButton.onclick = function(){
	...
	levelButton.innerHTML = "Уровень " + level;
};
...
toggleKeyboardControlButton.onclick = function(){
	if(codeRunning){
		console.error("Ошибка: Остановите выполняющийся скрипт, чтобы управлять роботом с клавиатуры.");
		return;
	}
...
command.onkeydown = function(e) {
  if(e.keyCode === 13) {
		...
		if(codeRunning){
			console.error("Ошибка: Выполнение команд во время работы скрипта приостанавливает скрипт.");
			return;
		}
